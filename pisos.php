...
<head>
    ...
    <title>Productos de Pisos</title>
...
<div class="container">
    ...
    $sql = "SELECT p.id, p.nombre, p.imagen, GROUP_CONCAT(c.color_nombre) AS colores 
            FROM productos p 
            LEFT JOIN colores c ON p.id = c.producto_id 
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            if (!empty($row['colores'])) {
                echo "<ul class='colors-list'>";
                $colores = explode(',', $row['colores']);
                foreach ($colores as $color) {
                    echo "<li class='color-item' style='background-color: $color'></li>";
                }
                echo "</ul>";
            }
            echo "</a>";
